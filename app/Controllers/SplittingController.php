...
class SplittingController extends BaseController
{
    public function index()
    {
        ...
        return view('transaction/splitting_view', $data);
    }
    ...
    {
        log_message('info', ' > saving splitting data');
        log_message('debug', print_r($this->request->getGet('plant'), TRUE));

        $item_data = Array();
        $plant = $this->request->getGet('plant');
        $pallet_source = $this->request->getGet('pallet_source');
        $pallet_dest = $this->request->getGet('pallet_dest');
        $rack_source = $this->request->getGet('rack_source');
        $rack_dest = $this->request->getGet('rack_dest');
        ...
        $current_stock = $model->get_current_stock($plant, $item_code, $lot_no, $rack_source, $pallet_source);

        log_message('debug', print_r($current_stock, TRUE));


        // move stock data //
        // $model->splitting($pallet_source, $pallet_dest, $rack_source, $rack_dest, $item_code, $lot_no, $qty, $username);

        if($model){
            $response['status'] = "ok";
            $response['message'] = "splitting data saved";
        }else{
