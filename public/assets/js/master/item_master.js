$(document).ready(function() {
    ...
    // activate datatable //
    var table = $('#table_item_master').DataTable( {
        ajax: {
            url: baseUrl + '/api?cmd=get_item_master',
            // "url": './api/master-api.php?cmd=get_item_master',
            "data": function(d) {
...
function toggle_item_master_upload(){
    $('#upload_item_master_container').toggle();
}


// toggle new item master form //
function toggle_new_item_master(){
    $('#new_item_master_container').toggle();
}


// - preview item master data text - //
function preview_item_master(){
    // get text data //
    var text_data_val = $('#text_item_master_data').val();
    // console.log(text_data_val);
    if(text_data_val.length > 0){
        $('.loading').show();
        $('#progress-text').html("Checking ...");
        var ajax = $.ajax({
                url: baseUrl + '/api?cmd=preview_item_master',
                // url: "./api/master-api.php?cmd=preview_item_master",
                method: "POST",
                data: {
        ...
        ajax.fail(function(err){
            ...
            display_error("Failed obtaining current STO period! <hr/> Invalid server response!. " + err.responseText)
        });
...
function upload_item_master(){
    if(confirm("Are you sure to upload this data?")){
        ...
        if(text_data_val.length > 0){
            ...
            var ajax = $.ajax({
                // url: "./api/master-api.php?cmd=upload_item_master",
                    url: baseUrl + '/api?cmd=upload_item_master',
                    method: "POST",
                    data: {
                        text_data: text_data_val
                    },
...
// - save new item master - //
function saveNewItemMaster(){
if(confirm("Create new item master?")){
    ...
    if(!form_invalid){
        var ajax = $.ajax({
                url: baseUrl + '/api?cmd=new_item_master',
                // url: "./api/master-api.php?cmd=preview_item_master",
                method: "POST",
                data: {
                    plant: plant_val,
                ...
                beforeSend: function () {
                    $("#new_item_master_loading").show().css("display", "inline-block"); // Show loading animation
                    // $("#table_container").hide(); // hide the table
                },
                complete: function () {
                    $("#new_item_master_loading").hide(); // Hide loading animation
                    // $("#table_container").show(); // show the table
                }
            });
