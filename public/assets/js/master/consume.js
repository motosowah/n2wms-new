$(document).ready(function(){
    ...
    // - activate datatable - //
    var table = $('#table_consume').DataTable( {
        ajax: {
            url: baseUrl + '/api?cmd=get_consume',
...
// toggle sap consume upload form //
function toggle_consume_upload(){
    $('#upload_consume_container').toggle();
}

// - preview sap consume data text - //
function preview_consume(){
    // get text data //
    var text_data_val = $('#text_consume_data').val();
    // console.log(text_data_val);
    if(text_data_val.length > 0){
        $('#btn_preview').disable();
        var ajax = $.ajax({
                url: baseUrl + '/api?cmd=preview_consume',
        ...
        ajax.done(function(msg){
            ...
            try {
                ...
                $('#text_consume_data').hide();
                // data valid //
                if(response['status'] == 'ok'){
                    // display output preview //
                    $('#sap_consume_upload_preview_output').html(response['data']);
                    ...
                    $('#sap_consume_upload_preview_output').html(response['message']);
...
// - upload sap consume data text - //
function upload_consume(){
    if(confirm("Are you sure to upload this data?")){
        ...
        if(text_data_val.length > 0){
            ...
            $('#btn_upload').disable();
            var ajax = $.ajax({
                    url: baseUrl + '/api?cmd=upload_consume',
                    method: "POST",
                    data: {
                        cmd: "consume_upload",
                        text_data: text_data_val
